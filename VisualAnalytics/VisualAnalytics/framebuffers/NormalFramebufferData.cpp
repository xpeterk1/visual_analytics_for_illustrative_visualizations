#include "NormalFramebufferData.h"

NormalFramebufferData::NormalFramebufferData(Model* model) : FramebufferData(model),
program(create_program("shaders/main.vert", "shaders/normal.frag")),
view_uniform(program, "view", model->view_matrix)
{
    ...
    normal_pixel_data.resize(3 * 600 * 800);
}

void NormalFramebufferData::refresh(glm::vec3 cameraPosition)
{
    ...
    glBindTexture(GL_TEXTURE_2D, color_texture);
    glGetTexImage(GL_TEXTURE_2D, 0, GL_RGB, GL_FLOAT, normal_pixel_data.data());
    ...
    for (int x = 0; x < 800; x++)
    {
        for (int y = 0; y < 600; y++)
        {
            float pixel = normal_pixel_data[3 * (y * 800 + x)];
            if (pixel != 0)
            {
                if (x < box.x) box.x = x;
                if (x > box.z) box.z = x;
...
NormalFramebufferData::~NormalFramebufferData()
{
